...
def NightyWeather():
    ...
    def update_city(value):
        ...
        if not api_key:
            print("API key required for city auto-complete.", type_="ERROR")
            return
        try:
            url = f"http://api.weatherapi.com/v1/search.json?key={api_key}&q={value}"
            response = requests.get(url, timeout=3)
            response.raise_for_status()
            suggestions = response.json()
            if not suggestions:
                print(f"No matches found for '{value}'. Try another name.", type_="ERROR")
                return
            selected = suggestions[0]
            full_city = f"{selected['name']}, {selected.get('region', '')}, {selected.get('country', '')}".strip(', ')
            update_setting("city", full_city)
            cache["data"] = None
            cache["timestamp"] = 0
            cache["call_count"] = 0
            cache["live_mode_warning_shown"] = False
            cache["call_limit_warning_shown"] = False
            save_cache(None, 0, 0, False, False)
            print(f"City set to '{full_city}' (first match). Weather data will refresh. 🏙️", type_="SUCCESS")
            if len(suggestions) > 1:
                alts = [f"{s['name']}, {s.get('region', '')}, {s.get('country', '')}".strip(', ') for s in suggestions[1:5]]
                print(f"Alternatives: {'; '.join(alts)}", type_="INFO")
        except Exception as e:
            print(f"City search error: {str(e)}. Using input as-is.", type_="ERROR")
    ...
    card.create_ui_element(UI.Input, label="City Search (auto-selects first match) 🏙️", show_clear_button=True, full_width=True, required=True, onInput=update_city, value=get_setting("city"))
    ...
    def get_time():
        try:
            data = fetch_weather_data()
            if not data or "location" not in data or "tz_id" not in data["location"]:
            ...
            target_time = datetime.now(timezone.utc).astimezone(target_tz)
            if time_format == "12":
                fmt = "%I:%M %p"
