@nightyScript(
    name="Weather & Time",
    author="Luhan",
    description="Fetches cozy weather and time for a city using WeatherAPI with a showcase image. Sign up at https://www.weatherapi.com/ for a free API key. Public version for nighty.one community."
)
def cityWeatherTime():
    import webbrowser
    import time
    ...
    CONFIG_PATH = f"{SCRIPT_DATA_DIR}/weatherTime.json"
    CACHE_PATH = f"{SCRIPT_DATA_DIR}/weatherTimeCache.json"
    ...
    def update_gmt_offset(selected):
        try:
            offset = float(selected[0])
            if -14 <= offset <= 14:
                update_setting("gmt_offset", offset)
                print("GMT offset updated! Time will refresh automatically. 🌍", type_="SUCCESS")
            else:
                print("GMT offset must be between -14 and +14.", type_="ERROR")
        except ValueError:
            print("Invalid GMT offset.", type_="ERROR")
    ...
    gmt_offsets = sorted([-12.0, -11.0, -10.0, -9.5, -9.0, -8.0, -7.0, -6.0, -5.0, -4.5, -4.0, -3.5, -3.0, -2.0, -1.0,
                          0.0, 1.0, 2.0, 3.0, 3.5, 4.0, 4.5, 5.0, 5.5, 5.75, 6.0, 6.5, 7.0, 8.0, 8.5, 9.0, 9.5,
                          10.0, 10.5, 11.0, 12.0, 12.75, 13.0, 14.0])
    offset_items = [{"id": str(off), "title": f"GMT{'-' if off < 0 else '+'}{abs(int(off)):02d}:{int((abs(off) - int(abs(off))) * 60):02d}"} for off in gmt_offsets]
    ...
    tab = Tab(name="WeatherTime", title="Weather & Time 🌦️", icon="sun")
    ...
    def get_time():
        try:
            gmt_offset = float(get_setting("gmt_offset") or 0.0)
            time_format = get_setting("time_format") or "12"
            if not -14 <= gmt_offset <= 14:
                raise ValueError("Invalid offset")
            utc_now = datetime.now(timezone.utc)
            target_time = utc_now + timedelta(seconds=int(gmt_offset * 3600))
            if time_format == "12":
                fmt = "%I:%M %p"
    ...
    print("Weather & Time running🌤️", type_="SUCCESS")
    tab.render()
